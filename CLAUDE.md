...
- `scripts/app.js` — all rendering + interactions (accounts with per-card
  ...
  **A pending slot pair is BROKEN by any other navigation** — a roster click, the
  ...
  second half of a stale pair, so it edits instead of charting. **Only POINTER
  clicks pair at all**: Enter/Space fire a synthetic click with `detail` 0, so
  two activations inside the window — or Enter auto-repeating while held — opened
  the editor, contradicting F2 being THE keyboard edit gesture. Otherwise clicking slot A, then a roster name, then slot A
