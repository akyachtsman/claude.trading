...
- **Watchlists panel** (`renderWatchlist()` + `wlTile()` + the editor, owner
  ...
  **Create and delete a whole list from the panel** (owner request 2026-08-01) —
  ...
  different jobs is how a control gets pressed by mistake; neither is gated on
  `wlLocked`, since that lock freezes **position only** and its own tooltip
  promises "adding and removing stay available" (the header `+`/🗑 already
  follow that rule) — the guard on delete is its confirm dialog, which names the
...
| S31 | Create + delete a whole list | With `?demo=1` NO `#wlNewListBtn` and no `.wl-del` render. Forced live+authed against a stateful fake roster: the created list PERSISTS to the store, a case-insensitive duplicate name is refused without writing and without discarding the typed name, the delete confirm names the list and its symbol count and opens focus on "Keep it", and the deleted list is gone from the store | A write control in demo, a duplicate accepted (it makes both lists unaddressable via `wlPick`), a confirm that doesn't say what is being destroyed, or a delete that only repaints |
