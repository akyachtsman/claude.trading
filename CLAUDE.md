...
| S46 | Heatmap label halo (rendered) | With `?demo=1`, open the heatmap and read the LIVE `#heatmapSvg` `<text>` nodes: at least 40 carry a PAINTED halo (a stroke colour AND a non-zero width AND opacity — `stroke-width: 0` renders nothing while every colour still reads correctly), every label sharing the tile-label ink is itself haloed, `paint-order` puts the stroke UNDER the glyph, and ink/halo clears 4.5. The ink is read off the render, never hardcoded. **This REPLACED a static assertion in `check-contrast.js`** (PR #283): the tile ramp is dynamic so the halo IS the AA mechanism, and five review rounds produced SEVENTEEN ways for valid source to satisfy a source-reading check while the rendered labels lost their halo — comments, decoy objects, nested keys, `Object.freeze` wrappers, string payloads, duplicate and computed keys, spreads, a renamed `stroke:`, `stroke-width: 0`, and finally an entirely UNUSED object literal of the right shape. The last two are why no source analysis can close this: a check satisfied by dead code is not a check. Retiring it returned `check-contrast.js` to the upstream template byte-for-byte | Fewer than 40 haloed labels (the halo removed outright — the FLOOR is what stops 'select the haloed ones, assert they are haloed' passing vacuously), a label with the tile ink and no halo (partial removal), a halo painted over the glyph, or ink/halo below AA |
