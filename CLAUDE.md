...
- `scripts/app.js` — all rendering + interactions (accounts with per-card
  ...
  **The re-fetch queue is DEDUPED** (Codex P2): a positional column can hold the
  same ticker in several slots, and an unresolvable one never lands in
  `wbRealSyms`, so a column repeating one bad symbol re-requested it once per
  slot on every cold start.
  **A pending slot pair is BROKEN by any other navigation** — a roster click or
  the header Load box resets `wbSlotClick` (Codex P2). Otherwise clicking slot A,
  then a roster name, then slot A again inside the window reads the last click as
  the second half of a pair: it opens A's editor and does NOT chart A, leaving
  the pane on the roster symbol rather than the newest thing asked for.
