...
- `scripts/app.js` — all rendering + interactions (accounts with per-card
  ...
  **A pending slot pair is BROKEN by any other navigation** — a roster click, the
  header Load box, and **an editor opening** all reset `wbSlotClick` (Codex P2,
  rounds 2 and 3). Otherwise clicking slot A, then a roster name, then slot A
